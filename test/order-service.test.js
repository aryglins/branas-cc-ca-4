...
test("Não deve fazer um pedido com cpf inválido: ", function () {
    ...
    expect(orderService.makeOrder(order)).toThrowError("CPF inválido");
});

test("Deve fazer um pedido com 3 itens (com descrição, preço e quantidade): ", function () {
	const order = new Order("123.456.789-09", [new Item("Notebook", 1, 2000), new Item("Mouse", 0.5, 2000), , new Item("Teclado", 0.25, 2000)], 0);
    const orderSummary = orderService.makeOrder(order);
...
test("Deve fazer um pedido com cupom de desconto (percentual sobre o total do pedido): ", function () {
	const order = new Order("123.456.789-09", [new Item("Notebook", 1, 2000), new Item("Mouse", 0.5, 2000), , new Item("Teclado", 0.25, 2000)], 0.1);
    const orderSummary = orderService.makeOrder(order);
    expect(orderSummary).toEqual(new OrderSummmary(order, 0, 3150));
});