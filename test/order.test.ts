...
test("Deve calcular o valor do frete com base nas dimensões (altura, largura e profundidade em cm) e o peso dos produtos (em kg)", () => {
    const order = new Order("123.456.789-09");
    order.addItem(new Item("Notebook", 1000, new ItemMeasures(20, 20, 3, 1)), 1);
    order.addItem(new Item("Monitor", 200, new ItemMeasures(30, 40, 5, 2.5)), 2);
    order.addItem(new Item("Mouse", 40, new ItemMeasures(10, 5, 2, 0.03)), 1);
    /*
    ...
    * Total: 10 + 2*25 + 10 = 70
    */
    expect(order.getShippingCost()).toBe(70);
});
